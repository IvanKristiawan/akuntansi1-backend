...
export const getLaporanBukuBesarLast = async (req, res) => {
  try {
    const laporanBukuBesar = await LaporanBukuBesar.find({kodeBukuBesar: req.params.id}).sort({ createdAt: -1 }).limit(1) 
    res.json(laporanBukuBesar);
  } catch (error) {
    // Error 500 = Kesalahan di server
    res.status(500).json({ message: error.message });
...
export const getLaporanBukuBesarByKodeBuku = async (req, res) => {
  try {
    const laporanBukuBesar = await LaporanBukuBesar.find({kodeBukuBesar: req.params.id});
    res.json(laporanBukuBesar);
  } catch (error) {
    // Error 500 = Kesalahan di server
    res.status(500).json({ message: error.message });
...
export const deleteLaporanBukuBesar = async(req, res) => {
    try {
        const deletedLaporanBukuBesar = await LaporanBukuBesar.deleteOne({ _id: req.params.id });
