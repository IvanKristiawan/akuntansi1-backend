...
export const getAJurnalUmumForDoc = async (req, res) => {
  try {
    const aJurnalUmum = await AJurnalUmum.find({}, {noJurnalUmum: 1, tanggal: 1, kodeAccount: 1, namaAccount: 1, keterangan: 1, debet: 1, kredit: 1, _id: 0});
    res.json(aJurnalUmum);
  } catch (error) {
    // Error 500 = Kesalahan di server
    res.status(500).json({ message: error.message });
...
export const getAJurnalUmumByNoNota = async (req, res) => {
  try {
    const aJurnalUmum = await AJurnalUmum.find({noJurnalUmum: req.params.id}, {noJurnalUmum: 1, tanggal: 1, kodeAccount: 1, namaAccount: 1, keterangan: 1, debet: 1, kredit: 1, _id: 0});
    res.json(aJurnalUmum);
  } catch (error) {
    // Error 404 = Not Found
...
export const deleteAJurnalUmum = async(req, res) => {
    try {
        const deletedAJurnalUmum = await AJurnalUmum.deleteOne({ _id: req.params.id });
        // Status 200 = Successful
