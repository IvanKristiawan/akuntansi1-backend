...
export const getKelompokBukuBesarKodeNama = async (req, res) => {
  try {
    const kelompokBukuBesar = await KelompokBukuBesar.find({}, {kode: 1, nama: 1, _id: 0});
    res.json(kelompokBukuBesar);
  } catch (error) {
    // Error 500 = Kesalahan di server
    res.status(500).json({ message: error.message });
...
export const updateKelompokBukuBesar = async (req, res) => {
  try {
    const updatedKelompokBukuBesar = await KelompokBukuBesar.findByIdAndUpdate(
      req.params.id,
      {
        $set: req.body,
      },
...
export const deleteKelompokBukuBesar = async(req, res) => {
    try {
        const deletedKelompokBukuBesar = await KelompokBukuBesar.deleteOne({ _id: req.params.id });
        // Status 200 = Successful
